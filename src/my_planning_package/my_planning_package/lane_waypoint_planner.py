...
class LaneWaypointPlanner(Node, LaneGraphPlanner):
    """
    ...
    def __init__(self, map_data, map_path, distance=3, map_flag=0):
        ...
        """
        Node.__init__(self, 'global_planner_node')
        LaneGraphPlanner.__init__(self, map_data, map_path, map_flag)
        self.get_logger().info("Route planner initialized...")
        """
        Self parameters
        """
        ...
        self.changeleft_subscriber = self.create_subscription(
            Bool,
            "/t4ac/decision_making/lane_change_order_left",
            self.change_left,
            10
        )
        self.changeright_subscriber = self.create_subscription(
            Bool,
        ...
        self.waypoint_publisher = self.create_publisher(
            ...
            "/t4ac/planning/route", 
            10)
        self.lane_change_executed_publisher = self.create_publisher(
            Bool, 
            "/t4ac/planning/lane_change_executed", 
            10)
    ...
    def publish_waypoints(self, route):
        ...
        self.get_logger().info(f"Published {len(path_msg.waypoints)} waypoints.")
...
def main(args=None):
    ...
    map_name = node.declare_parameter('t4ac/mapping/map_name', 'Town10HD').value
    map_path = node.declare_parameter("t4ac/mapping/map_path", "/home/renth/opendrive-mapping-planning/mapping_layer/maps/xodr/").value
    
    distance = float(node.declare_parameter('t4ac.mapping.distance_between_waypoints', 3.0).value)
    
    node.destroy_node()

    planner = LaneWaypointPlanner(map_name, map_path, distance)

    try:
        rclpy.spin(planner)
    except KeyboardInterrupt:
        planner.get_logger().info("Shutting down planner node.")
    finally:
