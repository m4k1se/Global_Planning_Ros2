...
def calculate_lanes(node, map_waypoints, map_kdtree, segment_index, waypoint_route, n1, n2):
    """
    ...
    monitor_lanes.header.stamp = node.get_clock().now().to_msg()
    for lane in lanes:
        monitor_lane = t4ac_msgs.msg.Lane()
        monitor_lane.right.way = lane.right_way
        monitor_lane.left.way = lane.left_way
...
def calculate_intersections(node, waypoint_route,segment_index, 
    ...
    if intersection_lanes:
        if get_lanes_as_dict:
            ...
            monitor_intersections.header.stamp = node.get_clock().now().to_msg()
            for lane in intersection_lanes:
